...
@section('title', 'Edit Curriculum Mapping')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Edit Curriculum Mapping</h1>
    <p class="text-gray-600 mt-1">{{ $mapping->grade_level }} - {{ $mapping->subject->subject_name }}</p>
</div>

<div class="bg-white rounded-lg shadow max-w-2xl">
    <form action="{{ route('admin.curriculum-mappings.update', $mapping) }}" method="POST" class="p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <select id="academic_year_id" 
                ...
                <option value="{{ $year->id }}" {{ old('academic_year_id', $mapping->academic_year_id) == $year->id ? 'selected' : '' }}>
                    {{ $year->year_label }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-6">
            ...
            <select id="grade_level" 
                ...
                <option value="{{ $level }}" {{ old('grade_level', $mapping->grade_level) === $level ? 'selected' : '' }}>
                    {{ $level }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-6">
            ...
            <select id="subject_id" 
                ...
                <option value="{{ $subject->id }}" {{ old('subject_id', $mapping->subject_id) == $subject->id ? 'selected' : '' }}>
        ...
        <div class="mb-6">
            ...
            <select id="is_required" 
                ...
                <option value="1" {{ old('is_required', $mapping->is_required) == 1 ? 'selected' : '' }}>Required</option>
                <option value="0" {{ old('is_required', $mapping->is_required) == 0 ? 'selected' : '' }}>Elective</option>
            </select>
            @error('is_required')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <input type="number" 
                   id="sequence_order" 
                   name="sequence_order"
                   value="{{ old('sequence_order', $mapping->sequence_order) }}"
                   min="0"
